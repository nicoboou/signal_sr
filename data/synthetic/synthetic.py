"""Simple factorial synthetic microscopy dataset."""
...
def sample_value(spec, rng, min_value=1.0):
    value = (
        rng.normal(spec.mean, spec.std) if isinstance(spec, NormalSpec) else float(spec)
    )
...
def make_cell_texture(size, cell_mask, period, amplitude, rng):
    ...
    noise = ndi.gaussian_filter(
        rng.normal(0, 1, (size, size)), sigma=max(1.0, period / 3)
    )
...
def make_parasite_mask(
    ...
    if (
        avoid_mask is not None
        and np.asarray(avoid_mask, dtype=bool).any()
        and min_distance > 0
    ):
...
def _draw_filament_candidate(size, cell_mask, cy, cx, width, length, rng):
    ...
    points = np.c_[
        np.clip(np.round(ys), 0, size - 1), np.clip(np.round(xs), 0, size - 1)
    ].astype(int)
...
def make_filament_mask(
    ...
    use_distance_constraint = (
        avoid_mask is not None and avoid_mask.any() and min_distance > 0
    )
    best_mask = None
    ...
    for _ in range(int(max_tries)):
        cy, cx = coords[rng.integers(0, len(coords))]
        candidate = _draw_filament_candidate(
            size, cell_mask, float(cy), float(cx), width, length, rng
        )
        if not candidate.any():
            continue

        candidate_distance = (
            mask_min_distance(candidate, avoid_mask)
...
def generate_synthetic_dataset(
    ...
    for parasite, filament, strong_texture in combinations:
        for _ in range(int(n_per_combination)):
            ...
            amplitude = (
                strong_texture_amplitude if strong_texture else poor_texture_amplitude
            )
            ...
            filament_mask = (
                make_filament_mask(image_size, cell_mask, filament_w, filament_l, rng)
                if filament
            ...
            parasite_filament_distance = (
                mask_min_distance(parasite_mask, filament_mask)
                if parasite and filament
                else np.nan
            )
            ...
            row = {
                ...
                "cell_mask_equivalent_diameter_px": equivalent_diameter(
                    int(cell_mask.sum())
                ),
                "parasite_filament_requested_min_distance_px": (
                    float(parasite_filament_min_distance_px)
                    if parasite and filament
                    else 0.0
                ),
